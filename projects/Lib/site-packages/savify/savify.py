...
class Savify:
    def __init__(self, api_credentials=None, quality=Quality.BEST, download_format=Format.MP3,
                 ...
                 ydl_options: dict = {}, skip_cover_art: bool = False, Logger: Logger = logging.basicConfig(
    format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',
    level=logging.INFO
),
    ...
    def download(self, query, query_type=Type.TRACK) -> None:
        ...
        if not (len(queue) > 0):
            logging.info('Nothing found using the given query.')

            return

        logging.info(f'Downloading {len(queue)} songs...')
    ...
    def _download(self, track: Track) -> dict:
        ...
        if check_file(output):
            logging.info(f'{str(track)} -> is already downloaded. Skipping...')
            status['returncode'] = 0
            return status
        ...
        if self.download_format != Format.MP3 or self.skip_cover_art:
            ...
            status['returncode'] = 0
            logging.info(f'Downloaded -> {str(track)}')
            return status
